...
- Until now, we've assumed (within each country), factors are mobiel
...
- Each country has two industries, .pink[manufacturing (m)] and .purple[agriculture (a)]

- Manufacturing is produced using capital and labor

- Agriculture is produced using land and labor

- Land and capital are .hi[specific factors], only used to produce one good

- Labor is the .hi[mobile factor] that can be used in *either* (or both) sectors
...
- Note $A (y)$ on top and $M (x)$ on bottom!<sup>.magenta[†]</sup>

.tiny[<sup>.magenta[†]</sup> This is because, if you think in our Ricardian terms, `\\(l_x=\frac{1}{MPL_x}\\)` and `\\(l_y=\frac{1}{MPL_y}\\)`, so `\\(\frac{l_x}{l_y} \implies \frac{MPL_y}{MPL_x}\\)`]
...
- Again, if you want to remember why, see my slides on [Factor Markets](https://ios20.classes.ryansafner.com/slides/1.8-slides.html#1)
]
]
...
# Our Two Country Trade Example: Specialization for Trade
...
.pull-left[
.smaller[
- Workers find their wage has increased (but less than the increase in relative price of M)
$$\frac{\Delta w}{w_1} < \cfrac{\Delta \left(\frac{P_M}{P_A}\right)}{\left(\frac{P_M}{P_A}\right)}$$

- Amount of manufactures $Q_M$ that can be purchased with wages has *fallen*!
  - .hi-purple[Real wage in terms of manufacturing, `\\(\frac{w}{p_M}\\)` decreases]

- Amount of manufactures $Q_A$ that can be purchased with wages has *risen*!
  - .hi-purple[Real wage in terms of agriculture, `\\(\frac{w}{p_A}\\)` increases]
...
- As more labor used in manufacturing, $MP_K$ rises. Each machine has more workers to work it.
...
- Labor is paid $w = MPL_M * p_M$
  - Rewrite as its real wage (in terms of $M)$: $\frac{w}{P_M}$
...
ggplot(data.frame(x=c(0,10)), aes(x=x))+
  ...
  geom_label_repel(data=tibble(x=1),aes(x=x),x=0.5, y=1, label="Gain to Capital Owners", color = "green", nudge_x = 0.5, nudge_y=0.75)+
  stat_function(fun=mp, geom="line", size=2, color="blue")+
  geom_label(aes(x=1.5,y=mp(1.5)), color = "blue", label=expression(MPL[M]), size = 4)+
  geom_segment(x=0, xend = 0.5, y=mp(0.5), yend=mp(0.5), size = 1, linetype="dotted")+
  geom_segment(x=0.5, xend = 0.5, y=mp(0.5), yend=0, size = 1, linetype="dotted")+
...
- As less labor used in agriculture, $MP_T$ falls. Each piece of land has fewer workers to work it.
...
- Labor is paid $w = MPL_A * p_A$
  - Rewrite as its real wage (in terms of $A)$: $\frac{w}{P_A}$
  - This times the total number of workers $L_A$ equals the total wages paid

- All residual income goes to land owners
]
]
...
ggplot(data.frame(x=c(0,10)), aes(x=x))+
  ...
  geom_label_repel(data=tibble(x=1),aes(x=x),x=0.5, y=1, label="Loss to Land Owners", color = "black", nudge_x = 0.5, nudge_y=0.75)+
  stat_function(fun=mp, geom="line", size=2, color="blue")+
  geom_label(aes(x=1.5,y=mp(1.5)), color = "blue", label=expression(MPL[A]), size = 4)+
  geom_segment(x=0, xend = 0.5, y=mp(0.5), yend=mp(0.5), size = 1, linetype="dotted")+
  geom_segment(x=0.5, xend = 0.5, y=mp(0.5), yend=0, size = 1, linetype="dotted")+
...
- .hi-purple[Factor specific to the sector whose relative price rises is better off.]
  - Capital for manufacturing
  
- .hi-purple[Factor specific to the sector whose relative price falls is worse off.]
  - Land for agriculture

- .hi-purple[The mobile factor is not clearly better or worse off.]
  - Labor
...
.pull-left[
.smaller[
- Workers find their wage has increased (but less than the increase in relative price of A)
$$\frac{\Delta w}{w_1} < \cfrac{\Delta \left(\frac{P_A}{P_M}\right)}{\left(\frac{P_A}{P_M}\right)}$$

- Amount of manufactures $Q_M$ that can be purchased with wages has *risen*!
  - .hi-purple[Real wage in terms of manufacturing, `\\(\frac{w}{p_M}\\)` increases]

- Amount of manufactures $Q_A$ that can be purchased with wages has *fallen*!
  - .hi-purple[Real wage in terms of agriculture, `\\(\frac{w}{p_A}\\)` decreases]
...
- As less labor used in manufacturing, $MP_K$ falls. Each machine has fewer workers to work it.
...
- As more labor used in agriculture, $MP_T$ rises. Each piece of land has more workers to work it.
...
- Labor: ambiguous
  - real wage rises in terms of $A$, falls in terms of $M$
...
- More .hi-purple[immobile] and .hi-purple[specific] workers lost income & jobs
  - Maybe geographically stuck in Detroit (family, friends, mortgage)
